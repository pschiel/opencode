...
export function Autocomplete(props: {
  ...
  const lspSummary = createMemo(() => {
    ...
    if (!data || data.length === 0) {
      return <text fg={theme.textMuted}>LSP: none</text>
    }
    return (
      <text fg={theme.textMuted}>
        ...
        {data.map((item, i) => {
          ...
          const busy = (item as { busy?: boolean }).busy === true
          const color =
            item.status === "error" ? theme.error : busy ? theme.warning : ready ? theme.success : theme.textMuted
          return (
            <>
              <span style={{ fg: color }}>•</span> {item.id}
              {i < data.length - 1 ? " " : ""}
            </>
          )
        })}
  ...
  function moveTo(next: number) {
    ...
    const headerLines = store.visible === "@" ? 1 : 0
  ...
  const height = createMemo(() => {
    ...
    positionTick()
    const extraLines = store.visible === "@" ? 1 : 0
